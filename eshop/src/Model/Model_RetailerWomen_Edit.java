...
public class Model_RetailerWomen_Edit {
	public static int Edit_Prdct(Retailer_Women_Prdct obj)
	{
		int i=0;
		Connection con=Conct.conect();
		
		
		String sql="Update retailer_productinsertwomen set ProductAddedOn=?, Retailer_Name=?, Type=?, BrandName=?,Fittings=?, Price=?, About=?,Tagline=? where Product_Id=?";
		try {
...
public static int Edit_Prdct_Image(Retailer_Women_Image ob) {
        ...
        try
        {
        	...
        	String sql2="Select count(Filename) from retailer_imageinsrt_women where Product_Id=? and ColorName=? and Filename=?";
        	int count=0;
        	...
        	{
            Connection con=Conct.conect();
        	String sql="Update retailer_imageinsrt_women set Image=?, Filename=? where Product_Id=? and ColorName=? and Filename=?";
        	PreparedStatement ps=con.prepareStatement(sql);
