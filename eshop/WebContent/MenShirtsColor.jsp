...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1" import="Bean.*,java.util.Calendar,java.sql.*"%>
...
<link rel="stylesheet" href="css1/body1style.css" type='text/css'
	media="screen" />
<!-- includes css data of tables in this page -->
...
<body onload="status()">
	...
	<div id="popupbox" class="LoginFormCx cf">
		...
		<li
			...
			style="color: white; text-decoration: none !important;" title="Close">X</a></li>
		<li style="margin-top: 4px; margin-left: -126px; margin-bottom: 10px;"><a
			href="#"
	...
	<%
		DateFormat df = new SimpleDateFormat("dd/MM/yy");
			...
			String delivery_date = ("Current date : " + (now.get(Calendar.DATE)) + "-" + (now.get(Calendar.MONTH) + 1)
			+ "-" + now.get(Calendar.YEAR));

			// add days to current date using Calendar.add method
			now.add(Calendar.DATE, 5);

			delivery_date = ((now.get(Calendar.DATE)) + "-" + (now.get(Calendar.MONTH) + 1) + "-"
			+ now.get(Calendar.YEAR));
	%>
	<div id="body1">
		...
		<div id="loginwrapper">
			<div id="topmessageB">NO DELIVERY CHARGES ON ORDERS ABOVE Rs500
				| EXTRA 5% OFF ON ORDERS ABOVE Rs1499</div>
	...
	<div id="carticon">
		<li><a href="View_Cart11.jsp" title="View Cart"><img
	...
	<div id="ShirtThumbTable" class="ShirtThumb1">
		...
		<table>
			...
			<%
				...
				String sql3 = "Select DISTINCT Filename from retailer_imageinsrt_men where Product_Id=?  AND ColorName=?";
				...
				while (rs3.next()) {
					if (flag == 0) {
			%>
			<tr style="z-index: 99; border-bottom: 2px solid white;">
				<td style="padding-left: 11.3%; position: static; z-index: 9999;" id="image"><img
					src="Image_diff_color?pid=<%=Product_id%> &ColorName=<%=Color%>"
			...
			%><tr>
				<td
					style="padding-top: 1.3%; padding-left: 2.7%; position: inherit; background: white;">
					<%
						} else {
					%> <%
 	String file = rs3.getString("Filename");
 %> <a
					href="MenShirtsRed1.jsp?Pid=<%=Product_id%> &Category=<%=category%> &Filename=<%=file%> &Price=<%=price%> &Colorname=<%=Color%>"><img
						src="Image_diff_color12?pid=<%=Product_id%> &ColorName=<%=Color%> &file=<%=file%>"
 ...
 %>
	...
	<div id="ColSizeTable">
		<div class="Details1">
			<table style="margin-left: 0px; margin-top: 3px; position: relative;"
				border="1">

				<%
					Connection con9 = Conct.conect();
					String sql9 = "Select  BrandName,Tagline,Price from retailer_productinsertmen where Product_Id=? ";
			...
			<table
				...
				<tr>
					<td><a href="#" style="color: olive;"><%=Color%></a>&nbsp <%
						...
						String sql4 = "Select DISTINCT ad1.ColorName,ad2.Product_Id  from retailer_colorinsertmen ad1 inner join retailer_imageinsrt_men ad2 on ad1.Product_Id=ad2.Product_Id and ad2.Product_Id=? and ad1.ColorName!=? ";
					...
					%><a
						href="MenShirtsColor.jsp?Pid=<%=Product_id%> &Colorname=<%=color%>  &Price=<%=price%> &Category=<%=category%>"><%=color%></a>
						<%
							}
						%></td>
				</tr>
				...
				<tr>
					<td>
						<%
							...
							String sql5 = "Select  Size from retailer_sizeinsrt_men where Product_Id=? and ColorName=?";
				...
				if(username!=null)
					...
					<form action="Cart" method="post">
						...
						<input type="hidden" name="color" value="<%=Color%>"> <input
							...
							style="margin-left: 60px; margin-top: 40px;"></input>
					</form>
					<% 
					   }  
	...
	<div class="PayDet1">
		<p style="margin: 0; padding: 0; color: #ccccff;">
			<i class="fa fa-money fa-lg" style="color: #00FF00;"></i> The maximum
			order value for 'Cash On Delivery' is <i class="fa fa-inr"></i>&nbsp50,000,
		...
		<p
			...
			<i class="fa fa-truck fa-lg" style="color: yellow;"></i> Your product
			will delivered on or before
			<%=delivery_date%>.
		</p>
		<br>
	</div>
	...
	<div id="ProDetTable" class="ProDet">
		...
		<table border="1" rules="rows"
			...
			<%
				...
				String sql6 = "Select Category, Type, BrandName, Fittings, Price, About, Tagline from retailer_productinsertmen where Product_Id=? ";
			...
			<tr>
				<td style="padding-left: 30%; font-weight: bold; padding-right: 0%;"
				...
				<div>
					...
					<form name="feedback" action="Feedback" method="post">
						...
						<br /> <input type="hidden" name="product_id"
							...
							name="colorname" value="<%=Color%>"> <input type="hidden"
